...
21.05.1990, Свердловск

| Телефон: | 8 (915) 37-84-826              |
| -------- | ------------------------------ |
| E-mail:  | diver841@gmail.com             |
| Сайт:    | https://vk.com/festralu        |
| GitHub:  | https://phenixflame.github.io/ |
| Telegram | https://t.me/an_fenix          |

## О себе

Есть несколько интересных проектов на github: [https://](https://phenixflame.github.io/)[**phenixflame.github.io**/](https://phenixflame.github.io/)
...
## **Собственные проекты**
 ...
 Проекты, на которые стоит обратить внимание. Сам код и описание размещены на [https://](https://phenixflame.github.io/)[**phenixflame.github.io**/](https://phenixflame.github.io/)
...
### **2019-2020 Младший программист Python**

ООО ТАРДИС. Стартап. Виджет предсказания размера одежды Sizolution.

Разрабатывал модуль интеграции с клиентами. В задачи модуля входило чтение и обработка данных клиентов (фид со списком продаваемой одежды в интернет-магазинах) по предоставляемым ими методам. Требовалось перевести разнообразные данные в формат используемый компанией, разметка данных (необходимо было по названиям вещей проставить пол вещи, а также теги типа футболка/джинсы), чтение и запись данных из PostgreSQL. 

Задачи на рабочем месте:

  - Валидация данных полученных от клиента. Обработка и разметка в нужный формат. Формирование рекомендаций по интеграции. Контроль за процессом интеграции и dataflow во всех частях системы(Фронт и несколько микросервисов в бэке)
  - Контроль качества размеченных данных и поиск различных багов
  - Устранение багов в бекэнде, связанных с данными и их обработкой
  - Поиск особенностей в данных, написание тестов для поиска багов и оценки качества интеграции, анализ процента вещей на к-х может работать система
  - Предложение идей по улучшению работы сервиса, а также различных метрик.

Реализованные проекты:

  - Рефакторинг сервиса интеграции: общий интерфейс для всех клиентов, с целью для разделения кода, который организует доступ к данным клиентам и кода, занимающегося разметкой. Полученные преимущества:
    - Сокращение объема кода для подключения клиента в три раза.
    - Локализация и унифицирование кода для разметки
    - Единый интерфейс клиентов, позволяющий проводить быстрое тестирование новых изменений, а также реализованного кода.

  - Разделение интерфейсов позволило рассматривать объекты размечающие данные отдельно от остального кода для тестирования и развития. Таким образом:
    - Возможность отдельного тестирования каждого улучшения связанного с разметкой. Возможность самостоятельного развития кода отдельно от клиентов.
    - Изменена базовая логика разметки (использованы регулярные выражения для разметки) — что повысило количество распознаваемых вещей с 80% до 98%. Точность распознавания выросла на 25%.
    - Улучшение разметки данных (было найдено 20% неправильно размеченных вещей из-за различных багов).

  - Написан специальный модуль обработки файлов с мерками. Решены проблемы:
    - У одного клиента может быть несколько разных форматов данных
    - У каждого клиента может быть несколько разных названий точек измерения, причем одно и то же название может соответствовать разным точкам измерения. (Например талия для футболки и для джинс с низкой посадкой)
    - Необходимо быстро собирать индивидуальный набор правил, который легко и быстро редактируется.
    - Необходимо быстро исследовать возможные названия мерок, отсеивать не нужные, фильтровать по значениям, допустимые границы которых зависят от разметки, находить признаки некорректных значений мерок в данных.

  - Тестирование данных, на которых работает система:
    - Модуль тестирования отслеживающий изменения в данных перед выкаткой изменений в production
    - Модуль вычисляющий основные метрики качества интеграции
    - Система тестов для контроля качества разметки данных, а также размечающего модуля

###	**2016-2019 Младший научный сотрудник**

ФГБНУ ТИСНУМ. Лаборатория Акустики.

- Разработка, изготовление и экспериментальное тестирование прототипов. Единоличная работа на всех (80%) этапах процесса. Являюсь одним из основных исполнителей проекта РНФ 16-12-10293.
- Самостоятельная инициатива в разработке ПО для автоматизации экспериментов (**VBA, LabView, C++, Arduino**). Производительность увеличена в 8-10 раз. Повысилась точность результатов. Появились новые возможности для анализа. Расширились границы применения приборов. (Например, измерения в реальном времени).
- Разработка гипотез, численный и теоретический расчет моделей резонаторов (**Макросы Excel, Wolfram Mathematica**). Самостоятельное исследование научно-технологических новинок и предложение улучшений. Разработка 2D чертежей (**AutoCad, Компас**).
- Обработка всего объема экспериментальных данных, их анализ и презентация (**OriginPro**). Подготовка материала для опубликования в журналах.
- Самостоятельное решение технологических проблем. Использование новых технологических подходов.
- Руководство студентами, постановка перед ними задач. Планирование работы.

### **2014-2016 Стажер-исследователь**  

ФГБНУ ТИСНУМ. Лаборатория Акустики. *Прохождение практики. Обзор литературы по теме работы, изготовление экспериментальных образцов, изготовление системы тестирования, снятие характеристик, предложения по улучшению результатов и решения технологических проблем.*

### **2011-2012 Аналитик-лаборант**  

Кардио-центре им. Мясникова. *Приготовление реактивов. Проведение лабораторных анализов. Обработка их результатов. Разработка предложений для увеличения точности анализа.*

### **2010-2011 Преподаватель**  

ЗФТШ при МФТИ. *Занятия по математике и физике 10 класс.. Организация материала уроков и планированиесамостоятельной работы учеников. Составление учебного плана.*

## Прочий опыт работы

Участие в работе студсовета факультета. Организация мероприятий (посвящение первокурсников и проч.). Староста учебной группы. Организация мероприятий в пределах группы.

Руководство горной секцией МФТИ. Заведование складом походного снаряжения, планирование бюджета склада, учет выдачи и приема снаряжения, организация закупок. Руководство спортивными горными и лыжными походами. Ведение методической деятельности. Ведение тренировочных выездов. Ведение тренировок.

Промышленный альпинизм.

Репетиторство по математике и физике. 3, 9, 10 класс. 5 учеников. Положительные отзывы за простоту и структурную подачу материала. Использование ассоциативных связей.

Продажа книг курьерным способом. Нет интереса к прямым продажам.

Организация бизнеса «Антикафе». Планирование мероприятий. Ведение настольных игр. Профессиональное ведение клуба спортивной игры «Мафия». Проведение чайных церемоний.
...
**Programming**: C++, VBA, LabView, Wolfram Mathematica, 

**Applications**: MS Exel(VBA макросы), MS Word, AutoCad, Компас 3D, Zotero|Mendeley, OriginPro. 

**Platforms**: уверенный пользователь OS Windows с начальными навыками администрирования
...
**1-е место** среди докладов секции «Перспективных углеродных материалов и технологий» на 61-й конференции МФТИ 2018г. 

Доклад

 Новосёлов А.С., Сорокин Б.П., Квашнин Г.М., Н.О. Асафьев. Разработка чувствительного элемента сенсора напыления тонких пленок на основе объемного акустического резонатора с алмазной подложкой. 
